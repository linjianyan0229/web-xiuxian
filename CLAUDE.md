...
- `users` 表字段：`id, dao_name(道号,唯一), email(唯一), password(bcrypt哈希), email_code(邮箱验证码), role(0玩家/1管理员), status(0禁用/1正常), realm_id(当前境界, 默认1=凡人), is_online(0离线/1在线), death_count(死亡次数), ling_shi(灵石), cultivation(修为), dao_yun(道韵), dao_law(道法领悟), comprehension(悟性%), avatar(头像访问路径, NULL=默认), register_time, login_time, last_sign_time, last_cultivate_time(修炼冷却), access_token`。
