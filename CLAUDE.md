...
面向人的完整技术文档在 `docs/技术文档.md`（架构/库表/API/机制/部署），改动表结构、接口或机制后请同步更新它与本文件。**`docs/` 已纳入版本库（随 GitHub 同步），但 `docs/记录/` 例外——已 gitignore 仅保留本地**（2026-07-10「删除历史记录」提交移除入库记录）。

**`docs/` 目录结构（本地文档统一归置）：**
- `docs/技术文档.md` — 面向人的完整技术文档。
- `docs/设计文档/` — **各系统机制设计稿/初稿**（修为修炼、境界突破、打坐、悟性、宗门、探索、功法等）。设计新机制先在此写文档，文档内互相引用用同目录相对路径 `./xxx.md`。
- `docs/记录/` — **改动/设计记录**（每次改动的总结 md，见下节；**仅本地保留，不入版本库**）。
...
- **社交：好友/私信/拉黑（已实装）**：三张表——
  - `friendships`（好友关系/请求）：`id, requester_id, addressee_id, status(0待通过/1已结交), created_time, updated_time`，同向唯一键 `(requester_id,addressee_id)` + **无向唯一键 `uk_pair_nodir(LEAST,GREATEST)`**（一对用户至多一行，需 MySQL ≥8.0.13 函数索引；老库迁移 `ensureFriendshipPairIndex()` 先清反向重复再补键——**保留 status 高者**[已结交 > 待通过，防降级]、同 status 保留 id 较小的先到者）。方向记发起/接收，是否好友需查两个方向；`findBetween` 先行拦截 + 并发互发时控制器捕获 `ER_DUP_ENTRY` 重查回执 409。
...
- **成丹材料与丹方表 `pill_materials` / `pill_recipes` / `pill_recipe_items`（数据+只读接口已实装）**：材料 748 种（22 个大境界档[凡人~大罗金仙] × 五类型 灵植/矿石/兽材/精魄/丹引 × 五档稀有度 凡/良/上/珍/绝品，带价格[5×1.55^档位×稀有度乘数 1/2.5/6/15/40]、产出地 origins JSON[凡俗/尘世/仙家三界域各 10 处]、`realm_rank`(1~22 大境界序号)与 `min_realm_id`(境界门槛)；名称唯一键）+ 丹方 435 张（`pill_recipes.pill_id` 主键 + 明细表一行一味[唯一键 `(pill_id,material_id)`，role=main/aux/catalyst+sort_order]；每丹 5~20 味随境界递增，结构=主药 1[上品+，类型呼应丹药类别]+辅药 n−2[本档+低一档]+丹引 1；**仙王及以上 40 种丹药无丹方**，接口 404「此丹无方」）。种子 `node/src/data/pill-materials.json`，`seedPillMaterials()` 幂等导入——材料判本表非空跳过，丹方**判主表与明细表任一为空即事务内清残局重导**（防主表提交后进程中断留下有丹方无材料的半截状态；明细 5530 条）；生成器 `资源文件/丹药相关/generate-materials.mjs`（确定性随机 mulberry32，同 generate-pools.mjs 模式，改配置重跑全量重建——**表已入库，重跑后需清表重导**）。数据访问 `models/pillMaterialModel.js`，控制器 `controllers/pillMaterialController.js`（全只读）：玩家 `GET /api/user/pills/:pillId/recipe`（丹方查阅）；后台 `GET /api/admin/materials(/meta|/:id)`（分页筛选 type/rarity/realm/keyword）+ `GET /api/admin/recipes` + `GET /api/admin/pills/:id/recipe`。材料背包/获得途径/炼丹玩法/后台管理页未实装。设计文档：`docs/设计文档/丹药材料与丹方机制.md`。
...
- **宗门设施门槛与宗门仓库（已实装）**：设施按**宗门人数**开启（`node/src/utils/sectFacilities.js`，前端镜像 `web/src/utils/sectFacilities.js`：仓库 0[直接拥有]/任务 5/藏经阁 10/商店 15/闭关室 20/执法堂 30/大牢 30/山峰 50，初稿可调；除仓库外本体未实装，前端「待启」，实装时服务端须复核门槛）。仓库两表：`sect_warehouses`（sect_id 主键, level 默认 1，**建宗事务建档 + 启动回填存量**——不惰性 INSERT，防解散并发下重建孤儿仓库，启动迁移并清理既有孤儿行）+ `sect_warehouse_items`（唯一键 `(sect_id,item_type,item_id,grade)` 叠加合并一行一格；item_type 现仅 pill，功法/法宝预留）。容量=100+(级-1)×50 格、满级 10；升级费=级×5000 灵石**扣操作者个人灵石**（宗门金库待做）。权限：查看/存入=本宗全体，**取出/升级=仓库管理权**（rank≤3 或 warehouse_elder，`canManageWarehouse()`）——控制器的事务外校验只为友好报错，**作准的复核在模型事务内**（`lockAndVerify` 按「sects → sect_members → sect_warehouses」顺序加锁重查宗门存在性/成员归属/管理权，防并发逐出/降职/解散间隙放行）。并发：存入事务先锁仓库行校验容量（新条目才占格）、取出数量守卫扣到 0 删行腾格、升级原子扣灵石+等级守卫。日志 type=`sect_wh_deposit/sect_wh_withdraw/sect_wh_upgrade`；解散宗门级联清两表。逻辑在 `models/sectWarehouseModel.js` + `controllers/sectWarehouseController.js`。设计文档：`docs/设计文档/宗门设施与仓库机制.md`。
...
- `GET /api/user/pills/:pillId/recipe` — 需用户鉴权，丹方查阅（图鉴/炼丹预览）→ `{ recipe }`（含 items 材料明细：名称/类型/稀有度/价格/产地/份数/角色）；丹药不存在或圣王以上无方 404
...
- `GET /api/user/sects/:id/warehouse?page&pageSize&keyword` — 需用户鉴权（本宗成员）→ `{ level, capacity, used, maxLevel, upgradeCost(满级 null), canManage, items:{list,total,page,pageSize} }` 宗门仓库概览+物品分页
- `POST /api/user/sects/:id/warehouse/deposit` — `{ pillId, grade, quantity }` 需用户鉴权（本宗成员），存入（从个人背包转移，新条目占格前校验容量）→ `{ ok }`；数量不足/满仓/宗门已散 409，已不在本宗 403（归属/宗门存在性在事务内复核）
- `POST /api/user/sects/:id/warehouse/withdraw` — `{ pillId, grade, quantity }` 需用户鉴权（仓库管理权），取出到操作者背包 → `{ ok }`；仓库不足/宗门已散 409，无权限/已不在本宗 403（权限在事务内复核）
- `POST /api/user/sects/:id/warehouse/upgrade` — 需用户鉴权（仓库管理权），修缮扩容（扣操作者个人灵石，费用=当前级×5000）→ `{ ok, level, capacity, user }`；灵石不足/等级变动/宗门已散 409，满级 400，无权限/已不在本宗 403（权限在事务内复核）
...
- `GET /api/admin/pills/:id/recipe` — 某丹药的丹方详情（材料明细连表）→ `{ recipe }`；无方 404
- `GET /api/admin/materials?page&pageSize&keyword&type&rarity&realm` → `{ list, total, page, pageSize }`（成丹材料列表；meta 路由需先于 `:id` 声明）
- `GET /api/admin/materials/meta` → `{ types, rarities, realms }`（筛选下拉用）
- `GET /api/admin/materials/:id` → `{ material }`；不存在 404
- `GET /api/admin/recipes?page&pageSize&keyword&realm&category` → `{ list, total, page, pageSize }`（丹方列表，连丹药名/境界/类型）
...
- **单一登录态**：整个前端只有一套 `token`/`user`（`stores/auth.js` + `api/http.js`，baseURL `/api`）。后台 API（`api/admin.js`）复用同一个 `http` 实例，不再有独立的 admin store/axios。`http` 响应拦截在 401 时清理登录态并跳 `/login`。**接口回抛最新用户视图时一律走 `auth.setUser(u)`**（赋值即写 localStorage）——直接 `auth.user = u` 不持久化，刷新会读到旧快照（曾致立派后 `/my-sect` 按旧 `sect_id` 误跳回列表）。
- 路由 `src/router/index.js`：`/login`、`/register`、`/reset`（重置密码，guestOnly）、`/home`（游戏）、`/sect`（宗门列表页）、`/my-sect`（我的宗门主页）、`/friends`（伙伴页）、`/pills`（丹药纳戒页）、`/equipment`（装备页）、`/shop`（商店/坊市页）、`/explore`（探索备程页），`/admin`（`AdminLayout` + 子路由 `dashboard`/`users`/`realms`/`pills`/`techniques`/`artifacts`/`sects`/`announcements`/`configs`）。**没有 `/admin/login`**——登录后 `LoginView` 按 `auth.user.role` 分流（管理员→`admin-dashboard`，玩家→`home`）。
...
- **我的宗门主页 `views/MySectView.vue`（独立路由页 `/my-sect`）**：**首页导航「宗门」按归属分流**——`auth.user.sect_id` 有值 → `/my-sect`，散修 → `/sect`；**拜入成功**（`SectDetailModal` 的 `joined` 事件）与**立派成功**均直接 push 到此页。背景 `web/image/zongmenbeijing.webp`（用户素材 2560×1440/527KB 直接引用，未再压制），水墨轻纱 + 锁一屏内滚（窄屏单列放开），已入 `GAME_PAGES`。布局：顶栏（返回洞府/宗门名/我的职位徽章/灵石）+ 左栏信息卡（宗徽、宗主、门人数、活跃度、立派/入宗时间、简介 + 操作按钮：门人名录[复用 SectMembersModal]、修订资料[rank≤3，复用 SectCreateModal 编辑模式]、天下宗门、退出宗门/解散宗门[宗主]）+ 右栏**设施宫格 8 卡三态**（门槛见 `utils/sectFacilities.js`：仓库「可用」点击开 `components/SectWarehouseModal.vue`[两页签 仓库/存入，行内数量面板取出/存入，顶部等级与「修缮扩容」按钮]；人数达标未实装「待启」；未达标灰化标「需 N 人」，点击提示当前人数——二期逐个实装时替换点击行为）。**归属兜底**：进页无 sect_id 或详情 `my` 为空（被逐/解散）→ 同步 profile 后 `router.replace('/sect')`；人事变动（名录 `changed`）后刷新 profile+详情（宗主可能易位、我可能被改任）。
- **伙伴页 `views/FriendsView.vue`（独立路由页 `/friends`，非弹窗）**：首页左侧导航「伙伴」进入，同宗门页水墨壳（dongfu 背景、锁一屏内滚、已入 `App.vue` 的 `GAME_PAGES` 获滑动过渡）。**经典 IM 双栏布局**——左栏列表（四页签：好友[在线态+未读角标]、群聊[占位「敬请期待」]、结交请求[应允/婉拒]、黑名单[解除]，请求/黑名单页签带数量角标；好友页签下方另有**「陌生传音」分组**=有私信往来但非好友的会话，点开可读可回——后端允许向任意玩家传音、断交后会话脱离好友列表，无此分组这些未读只有角标没有入口；**已拉黑对象**在分组内标「已拉黑」、会话可读但输入区禁用[发送必被后端 403]，顶部按钮切为「解除拉黑」），右栏为**内嵌私聊区**：点左栏好友即 `selectPeer` 载入会话（消息滚动区 + 顶部对方信息与断交/拉黑操作 + 底部发送区），发送/5 秒轮询/进会话标记已读均在页内完成，未选会话时右栏显示引导空态。进页 `apiFriends/apiBlocks/apiConversations` 并发拉取，好友行未读数取自会话列表 `unreadMap`。**私信会话弹窗 `components/PrivateChatModal.vue`**（同款一对一传音逻辑）仅由**世界频道名片**的「私信」按钮使用（伙伴页用内嵌聊天，不走弹窗）。**频道名片 `ChatUserModal.vue` 右侧信息栏加四按钮**：结交（按关系态切换 结交/接受结交/待通过/已是道友）、切磋（占位 toast）、私信（拉黑关系下禁用，`@private` 抛给父级开 PrivateChatModal）、拉黑（拉黑/解除拉黑，`window.confirm` 二次确认）——打开时按 `user_id` 拉 `apiRelation` 判定按钮态。**首页「伙伴」导航红点角标**：未读私信数 + 待处理结交请求数（`HomeView` 进页拉取、每 30 秒背景刷新；`socialBadge`）。
- **丹药纳戒页 `views/PillsView.vue`（独立路由页 `/pills`，非弹窗，取代旧 `PillBagModal.vue`）**：首页左侧导航「丹药」进入，同宗门/伙伴页水墨壳（dongfu 背景、锁一屏内滚、已入 `GAME_PAGES` 获滑动过渡）。列表列：品质标签/丹药名/品阶(境界)/类型/功效(摘要，无摘要时由 effects 数组拼简述)/数量 + 行内三操作按钮**服用**（占位 toast）/**赠送**/**丢弃**；筛选：品质(凡/灵/道)/品阶(境界)/类型 下拉 + 名称关键字，下拉走 `/api/user/pills/meta` 仅含实际持有；分页，变动后本页清空自动回退一页。行点击打开 `components/PillDetailModal.vue` 详情（药力逐条+摘要/备注），行内「赠送/丢弃」经其新增 `initialMode` 属性直达对应面板（`main|gift|discard`），`changed` 回抛剩余数量，扣到 0 关详情窗并刷新列表。窄屏（≤860px）列表在 `.list-wrap` 内 `overflow-x:auto` 横滚（行 `min-width:500px` 保证表头/行/底线同宽、露 6px 横滚条提示、**操作列 `position:sticky; right:0` 吸附可视区右缘始终可点**），页面本身不出横向滚动条。
- **装备页 `views/EquipmentView.vue`（独立路由页 `/equipment`）**：首页左侧导航「装备」进入（导航项排在「法宝」后；常用功能的「吐纳」已移除），同款水墨壳、已入 `GAME_PAGES`。左栏人物面板：性别立绘（boy/girl.webp）+ 覆盖槽位——图中央「心法」、图左「装备」、图右「武器/法宝」，立绘下方横排三个功法槽（1 主 + 2 辅）；**穿戴系统未实装，槽位全部为占位**（虚线金框空态，点击 toast），待 `user_techniques`/`user_artifacts` 落地后填充。右栏「纳戒储物」：**列表展示**（表头 品质/物品/品阶/类型/数量，窄屏 ≤860px 收起品阶/类型列），接真实丹药背包（`apiMyPills` pageSize=100，超量提示去丹药页），整行可点开 `PillDetailModal`，变动后刷新。功法/法宝入背包后需并入此列表视图。
- **商店页 `views/ShopView.vue`（独立路由页 `/shop`，占位骨架）**：首页左侧导航「商店」进入（导航项在「丹药」与「宗门」之间），同款水墨壳（背景 `web/image/shangdian.webp`，用户素材 282KB 直接引用未压制）、已入 `GAME_PAGES`。顶栏含灵石余额徽章；主体为四个店铺分区占位卡（丹阁/器阁/书斋/杂货铺，标「待启」，点击 toast）。**商店本体未实装**（商品表/上架/灵石购买结算待设计），实装时替换 `openShop` 点击行为并接后端接口。
- **探索页 `views/ExploreView.vue`（独立路由页 `/explore`，前端先行）**：首页导航「探索」进入，同款水墨壳（背景 `web/image/tansuo.webp`，用户素材 296KB 直接引用未压制）、已入 `GAME_PAGES`。按探索机制设计稿 §7.5「备程页（先投入后揭晓）」做骨架：左栏时长四档单选（与打坐同档 30/60/120/240 分钟）+ 加持灵石可选输入 + 「出发」按钮（占位 toast），右栏行前须知。**后端探索链路未实装**（建表/接口/结算/DeepSeek 预生成均无，见 `docs/设计文档/探索机制.md`），实装时替换 `setOut()` 并补「探索中倒计时/结算纪要」页态、时长档改从状态接口取（同打坐 options 模式）。
...
- **弹窗可访问性 `composables/modalA11y.js`**：`useModalA11y(() => props.visible, () => emit('close'))` 返回 `dialogRef`——提供焦点圈定（Tab 循环、隐藏/disabled 元素不参与）、Esc 关闭、打开时焦点移入弹窗本体、关闭后焦点恢复到触发元素。模板在弹窗容器上挂 `ref="dialogRef" role="dialog" aria-modal="true" aria-labelledby="<标题id>" tabindex="-1"`（样式补 `:focus { outline: none }`）。目前已接入 `SectWarehouseModal`，**其余弹窗有同类问题待逐个接入**。
