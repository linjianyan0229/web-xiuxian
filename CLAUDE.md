...
- 路由 `src/router/index.js`：`/login`、`/register`、`/reset`（重置密码，guestOnly）、`/home`（游戏）、`/sect`（宗门列表页）、`/my-sect`（我的宗门主页）、`/friends`（伙伴页）、`/pills`（丹药纳戒页）、`/equipment`（装备页），`/admin`（`AdminLayout` + 子路由 `dashboard`/`users`/`realms`/`pills`/`techniques`/`artifacts`/`sects`/`announcements`/`configs`）。**没有 `/admin/login`**——登录后 `LoginView` 按 `auth.user.role` 分流（管理员→`admin-dashboard`，玩家→`home`）。
...
- **装备页 `views/EquipmentView.vue`（独立路由页 `/equipment`）**：首页左侧导航「装备」进入（导航项排在「法宝」后；常用功能的「吐纳」已移除），同款水墨壳、已入 `GAME_PAGES`。左栏人物面板：性别立绘（boy/girl.webp）+ 覆盖槽位——图中央「心法」、图左「装备」、图右「武器/法宝」，立绘下方横排三个功法槽（1 主 + 2 辅）；**穿戴系统未实装，槽位全部为占位**（虚线金框空态，点击 toast），待 `user_techniques`/`user_artifacts` 落地后填充。右栏「纳戒储物」：8 列格子（窄屏 4 列）接真实丹药背包（`apiMyPills` pageSize=100，超量提示去丹药页），物品格带品质角标/名称/数量、空格补位凑 40 格，点击开 `PillDetailModal`，变动后刷新。功法/法宝入背包后需并入此格子视图。
