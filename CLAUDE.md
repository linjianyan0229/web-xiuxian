...
- 路由 `src/router/index.js`：`/login`、`/register`、`/reset`（重置密码，guestOnly）、`/home`（游戏）、`/sect`（宗门列表页），`/admin`（`AdminLayout` + 子路由 `dashboard`/`users`/`realms`/`pills`/`sects`/`configs`）。**没有 `/admin/login`**——登录后 `LoginView` 按 `auth.user.role` 分流（管理员→`admin-dashboard`，玩家→`home`）。
