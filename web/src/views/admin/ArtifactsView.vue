...
const type = ref('')
const tier = ref('')
const meta = ref({ tiers: [], types: [] })
const realmOptions = ref([]) // 全部境界，客户端聚合出大境界名供装备门槛下拉
const loading = ref(false)

const TYPE_OPTIONS = [
  { value: 'natal', label: '本命法宝' },
  { value: 'offense', label: '攻击法宝' },
  { value: 'defense', label: '防御法宝' },
  { value: 'support', label: '辅助法宝' },
]
const TIER_OPTIONS = [
  { value: 'faqi', label: '法器' },
  { value: 'lingqi', label: '灵器' },
  { value: 'baoqi', label: '宝器' },
  { value: 'daoqi', label: '道器' },
  { value: 'xianqi', label: '仙器' },
]
...
// 大境界名去重（保持境界表顺序），供装备境界要求下拉
const majorRealms = computed(() => {
  const seen = new Set()
  const out = []
  for (const r of realmOptions.value) {
    if (!seen.has(r.realm)) {
      seen.add(r.realm)
      out.push(r.realm)
    }
...
async function load() {
  loading.value = true
  try {
    const res = await apiAdminArtifacts({
      page: state.page,
...
async function loadMeta() {
  try {
    meta.value = await apiArtifactMeta()
  } catch {
    /* 元数据失败不影响列表 */
  }
  try {
...
function openCreate() {
  ...
  Object.assign(form, {
    ...
    thresholdBase: 100,
    thresholdRatio: 2,
    effects: [{ target: 'attack', type: 'percent', value: 5, polarity: 'positive' }],
    summary: '',
    intro: '',
  })
  formError.value = ''
...
function openEdit(row) {
  ...
  Object.assign(form, {
    ...
    realmReq: row.realm_req || '',
    refineMax: row.refine_max,
    multipliersText: parseJsonCol(row.refine_multipliers, []).join(', '),
    refineBase: row.refine_base,
...
function buildPayload() {
  ...
  return {
    ...
    refineMax: Number(form.refineMax),
    refineMultipliers: multipliers,
    refineBase: Number(form.refineBase),
...
async function saveForm() {
  ...
  try {
    ...
    if (editRow.value) {
      await apiUpdateArtifact(editRow.value.id, payload)
      toast.success('法宝已更新')
    } else {
      await apiCreateArtifact(payload)
      toast.success('法宝已创建')
    }
...
async function confirmDelete() {
  ...
  delBusy.value = true
  try {
    await apiDeleteArtifact(delRow.value.id)
  ...
  <section>
    <div class="head">
      ...
      <div class="tools">
        ...
        <div class="search">
          <input v-model="keyword" type="text" placeholder="搜索法宝名" @keyup.enter="onSearch" />
          <button class="btn-sm" @click="onSearch">搜索</button>
        </div>
        <button class="btn-sm" @click="openCreate">新增法宝</button>
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr v-if="!loading && state.list.length === 0">
            <td colspan="7" class="empty">暂无法宝</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div class="pager">
      <span class="total">共 {{ state.total }} 件法宝</span>
      <div class="ctrl">
    ...
    <div v-if="formVisible" class="mask" @click.self="formVisible = false">
      <div class="modal wide">
        <header class="m-head">
          <h3>{{ editRow ? `编辑法宝 · ${editRow.name}` : '新增法宝' }}</h3>
        ...
        <div class="m-body">
          <div class="grid-2">
            ...
            <label class="fld">
              <span>装备境界要求（大境界）</span>
              <select v-model="form.realmReq" :disabled="formBusy">
                <option value="">无要求</option>
                <option v-for="r in majorRealms" :key="r" :value="r">{{ r }}及以上</option>
              </select>
            </label>
            <label class="fld">
              <span>分类/定位</span>
              <input v-model.trim="form.category" placeholder="如 攻击槽" :disabled="formBusy" />
            </label>
            <label class="fld">
              <span>满炼化层数（1~6）</span>
              <input v-model.number="form.refineMax" type="number" min="1" max="6" :disabled="formBusy" />
            </label>
            <label class="fld">
              <span>炼化倍率（逗号分隔，长度=满层数）</span>
              <input v-model="form.multipliersText" placeholder="1, 1.5, 2.2" :disabled="formBusy" />
            </label>
            <label class="fld">
              <span>温养炼化基数（每小时）</span>
              <input v-model.number="form.refineBase" type="number" min="1" :disabled="formBusy" />
            </label>
            <label class="fld">
              <span>炼化阈值基数 / 公比</span>
              <div class="pair">
          ...
          <div class="fx-edit">
            <div class="fx-head">
              <span>基础五维加成（正面随炼化倍率放大、负面恒定）</span>
            ...
            <div v-for="(e, i) in form.effects" :key="i" class="fx-row">
              ...
              <select v-model="e.polarity" :disabled="formBusy">
                ...
                <option value="negative">负面（反噬）</option>
              </select>
...
.fx { font-size: 13px; color: var(--muted); max-width: 280px; }
